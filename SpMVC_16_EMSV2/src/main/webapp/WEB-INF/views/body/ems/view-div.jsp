<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<html>
<style>
.email-view {
	display: flex;
	flex-flow: column;
	width: 95%;
	margin: 10px auto;
}

.email-view div {
	display: flex;
	flex-wrap: nowrap;
	width: 95%;
	
	margin: 1px auto;
	border: 1px solid #ff7473;
	padding: 5px;
}

.email-view div div:nth-child(odd) {
	width: 30%;
	background-color: #ff7473;
	padding: 5px;
	text-align: right;
	margin-right: 5px;
	font-weight: bold;
	color: white;
}

.email-view div div:nth-child(even) {
	width: 65%;
	
	padding: 5px;
}

.email-view div:nth-child(4) div,.email-view div:nth-child(5) div{
	width: 100%;
	padding: 10px 16px;
	background-color: #ff7473;
	color: white;
	text-align: left;
}


.email-view div:last-child {
	display: flex;
	border:0;
}

.email-view div:last-child button {
	padding:10px 16px;
	margin:5px;
	color: white;
	cursor: pointer;
	border: 0;
	outline: 0;
	border-radius: 5px;
}



.email-view div:last-child button:first-child {
	margin-left: auto;
}

.email-view div:last-child button:nth-child(1) { background-color: #ff7473 }
.email-view div:last-child button:nth-child(2) { background-color: #ff7473 }
.email-view div:last-child button:nth-child(3) { background-color: #ff7473; color:white}
</style>

<script>
$(function(){

	var buttonText = Array("수정","삭제","리스트보기")
	
	let buttons = $(".email-view div:last-child").children()
	for(let i = 0 ; i < 3 ; i++) {
		buttons.eq(i+1).text(buttonText[i])	
	}
	// buttons.eq(2).text(buttonText[1])
	// buttons.eq(3).text(buttonText[2])
	
	$(".email-view div:last-child button").click(function(){
	
		let text = $(this).text()
		let url = "${rootPath}/ems/"
		if(text == buttonText[0]) {
			url += "update/" + ${emailVO.ems_seq}
		} else if(text == buttonText[1]) {
			url += "delete/" + ${emailVO.ems_seq}
		} else if(text == buttonText[2]) {
			url += "list"
		}
		document.location.href = url
		
	})
	
	
	
})



</script>

<article class="email-view">
	<div>
		<div>받은Email</div>
		<div>${emailVO.to_email}</div>
		<div>받은사람</div>
		<div>${emailVO.to_name}</div>
	</div>
	<div>
		<div>보낸Email</div>
		<div>${emailVO.from_email}</div>
		<div>보낸사람</div>
		<div>${emailVO.from_name}</div>
	</div>
	<div>
		<div>보낸날짜</div>
		<div>${emailVO.send_date}</div>
		<div>보낸시각</div>
		<div>${emailVO.send_time}</div>
	</div>
	<div>
		<div>${emailVO.subject}</div>
	</div>
	<div>
		<div>${emailVO.content}</div>
	</div>
	<div>
		<button>수정</button>
		<button>삭제</button>
		<button>리스트</button>
	</div>
</article>
</html>